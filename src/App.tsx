...
import { MapPin, Bell, Navigation, Settings, AlertCircle, Volume2, VolumeX, Search, History, Trash2, Share2, Check } from 'lucide-react';
...
export default function App() {
  ...
  const addStop = async (lat: number, lng: number, name: string) => {
    let stopName = name;
    if (!stopName) {
      stopName = await reverseGeocode(lat, lng);
    }
    
    setDestinations(prev => [...prev, [lat, lng]]);
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                      <motion.div 
                        ...
                        initial={{ width: 0 }}
                        animate={{ 
                          width: `${Math.max(0, Math.min(100, (1 - (distanceToDest * 1000 - radius) / 2000) * 100))}%` 
                        }}
                      />
                    </div>
