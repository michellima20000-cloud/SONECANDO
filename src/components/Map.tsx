...
function RoutingMachine({ start, waypoints, onRouteInfo, onRouteCoordinates }: { 
  ...
  useEffect(() => {
    if (!map || !start || waypoints.length === 0) {
...
export default function Map({ currentLocation, destinations, onDestinationSelect, onRadiusSelect, onRouteInfo, onRouteCoordinates, radius, currentStopIndex, transportMode = 'car' }: MapProps) {
  ...
  return (
    <div className="h-full w-full rounded-xl overflow-hidden shadow-lg border border-slate-200 relative">
      ...
      >
        ...
        />
        <RecenterMap center={currentLocation && destinations.length === 0 ? currentLocation : null} />
      </MapContainer>
    </div>
  );
}
